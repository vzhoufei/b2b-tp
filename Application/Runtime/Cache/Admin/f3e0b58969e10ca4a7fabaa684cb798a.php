		              ...
		              <table id="list-table" class="table table-bordered table-striped dataTable">
						...
						<tbody>
						  <?php if(is_array($prom_list)): foreach($prom_list as $k=>$vo): ?><tr role="row" align="center">
							 ...
							 <th><img width="20" height="20" src="/Public/images/<?php if($vo[recommend] == 1): ?>yes.png<?php else: ?>cancel.png<?php endif; ?>" onclick="changeTableVal('flash_sale','id','<?php echo ($vo["id"]); ?>','recommend',this)"/></th>
							 <td>
							 	<?php if($vo['status'] == 0): ?><a href="javascript:;" class="btn btn-success" onclick="changeStatus(1,'<?php echo ($vo["id"]); ?>','flash_sale')">通过</a>
		                      		<a href="javascript:;" class="btn btn-warning" onclick="changeStatus(2,'<?php echo ($vo["id"]); ?>','flash_sale')">拒绝</a><?php endif; ?>
		                      	<?php if($vo['status'] == 1): ?><a class="btn btn-warning" href="javascript:;" onclick="changeStatus(3,'<?php echo ($vo["id"]); ?>','flash_sale')">取消</a><?php endif; ?>
